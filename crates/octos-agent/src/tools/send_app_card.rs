...
//! channel (`octos-bus::matrix_channel::send_matrix_message`) extracts
//! that key and copies it verbatim into the outgoing Matrix event
//! `content`. Other channels (CLI / Telegram / Feishu) ignore the
//! metadata and deliver only the `body` text.
...
struct Input {
    ...
    /// `"weather"` is supported by Robrix. Unknown types are rendered as
    /// plain `body` text on the client with a warning.
    r#type: String,
...
mod tests {
    ...
    #[tokio::test]
    async fn valid_weather_payload_puts_org_octos_app_in_metadata() {
    ...
    #[tokio::test]
    async fn non_object_initial_state_fails() {
        let (tool, _rx) = make_tool();
        let result = tool
            .execute(&json!({
                "type": "weather",
            ...
            .await;
        // Serde rejects non-object initial_state because the struct field is
