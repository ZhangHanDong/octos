...
impl Agent {
                ...
                loop {
                    ...
                    match response.stop_reason {
                        ...
                        StopReason::ToolUse => {
                            ...
                            if let Some(draft) = pending_approval {
                                self.emit_cost_update(turn.total_usage(), &response.usage);
                                return Ok(ConversationResponse {
                                    content: String::new(),
                                    reasoning_content: None,
    ...
    async fn handle_tool_use(
        ...
        let mut pending_approval: Option<crate::approval::PendingApprovalDraft> = None;
        for batch in tool_batches {
            let mut batch_response = limited_response.clone();
            batch_response.tool_calls = batch.to_vec();
            match self.execute_tools(&batch_response).await? {
                ...
                ToolExecutionOutcome::ApprovalRequested(draft) => {
                    pending_approval = Some(draft);
                    break;
                }
            }
